...
export const TelegramLoader: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto my-12 p-6 sm:p-8 bg-[#FAF8F5] border border-[#1A1A1A]">
      {/* Telegraph Header */}
      <div className="border-b border-[#1A1A1A] pb-3 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs font-mono">
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 bg-[#B91C1C] animate-pulse" />
          <span className="font-bold uppercase tracking-widest text-[#1A1A1A]">
            CONFIDENTIAL TELEGRAM WIRE
          </span>
        </div>
        <div className="text-[#8C8477] uppercase text-[11px]">
      ...
      <div className="min-h-[140px] flex flex-col justify-center">
        <div className="text-[11px] font-mono text-[#8C8477] mb-2 uppercase tracking-wider">
          STATUS MONITOR [STAGE 0{currentLineIndex + 1}/06]:
        </div>
        <p className="font-mono text-sm sm:text-base text-[#1A1A1A] leading-relaxed tracking-wide">
          <span className="text-[#B91C1C] font-bold">&gt;&gt; </span>
          {displayText}
          <span className="inline-block w-2.5 h-4 ml-1 bg-[#1A1A1A] align-middle animate-blink" />
        </p>
      </div>

      {/* Telegram Footer Rule */}
      <div className="mt-6 pt-3 border-t border-[#D8D2C7] flex justify-between items-center text-[10px] font-mono text-[#8C8477] uppercase tracking-wider">
