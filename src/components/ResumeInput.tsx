...
export const ResumeInput: React.FC<ResumeInputProps> = ({ onSubmit, isLoading }) => {
  ...
  return (
    <section className="w-full max-w-4xl mx-auto px-4 sm:px-6 py-8 md:py-12">
      {/* Editorial Headline */}
      <div className="mb-8 text-center sm:text-left border-b border-[#D8D2C7] pb-6">
        <div className="text-[11px] font-mono tracking-widest text-[#8C8477] uppercase mb-2 flex items-center gap-2">
          <span>SECTION 01</span>
          <span>•</span>
          <span>PRIMARY INTAKE</span>
        </div>
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-bold tracking-tight text-[#1A1A1A] leading-tight">
          Let’s See What You’re Working With.
        </h2>
        <p className="mt-3 text-sm sm:text-base font-serif text-[#5C5855] leading-relaxed max-w-3xl">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 text-xs font-mono">
          <span className="text-[#8C8477] uppercase text-[10px] tracking-wider">Test Drive With Samples:</span>
          {SAMPLE_RESUMES.map((sample, idx) => (
            <button
              key={sample.label}
              type="button"
              onClick={() => handleSampleSelect(idx)}
              className="px-2.5 py-1 bg-[#EFEAE1] hover:bg-[#1A1A1A] hover:text-[#F7F5F0] border border-[#D8D2C7] text-[#1A1A1A] transition-colors cursor-pointer text-[11px]"
            >
              + {sample.label}
            </button>
          ))}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div
          ...
          className={`border border-dashed p-4 sm:p-5 transition-colors cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
            isDragging
              ? "border-[#B91C1C] bg-[#FAF5F0]"
              : "border-[#A39D93] bg-[#FAF8F5] hover:border-[#1A1A1A]"
          }`}
        >
          <input
            ref={fileInputRef}
          ...
          <div className="flex items-center gap-3">
            <div className="p-2 border border-[#D8D2C7] bg-[#F7F5F0] text-[#1A1A1A]">
              <Upload size={18} />
            </div>
            <div>
              ...
              <div className="text-[11px] font-serif text-[#5C5855]">
                {fileName
          ...
          {fileName && (
            <button
              ...
              className="text-[11px] font-mono text-[#8C8477] hover:text-[#B91C1C] flex items-center gap-1 self-start sm:self-auto cursor-pointer"
            >
        ...
        {uploadError && (
          <div className="p-3 border-l-2 border-[#B91C1C] bg-[#FAF0F0] text-[#B91C1C] text-xs font-mono flex items-center gap-2">
            <AlertCircle size={15} />
        ...
        <div className="relative">
          <div className="flex justify-between items-end pb-2 border-b border-[#1A1A1A]">
            <label
              htmlFor="resume-text"
              className="text-xs font-mono uppercase tracking-wider text-[#1A1A1A] font-bold"
            >
              Résumé Transcript / Text Copy
            </label>
            <div className="text-[11px] font-mono text-[#8C8477] flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-b border-[#D8D2C7] pb-6">
          <div className="md:col-span-1">
            <label
              htmlFor="target-role"
              className="block text-xs font-mono uppercase tracking-wider text-[#1A1A1A] font-semibold mb-1"
            >
              Target Role (Optional)
            </label>
            <p className="text-[11px] font-serif text-[#5C5855] leading-snug">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
          <div className="text-[11px] font-mono text-[#8C8477] uppercase tracking-wider flex items-center gap-1.5">
            <span className="w-2 h-2 bg-[#B91C1C] inline-block" />
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            {resumeText && (
              <button
                type="button"
                onClick={handleClear}
                className="px-4 py-3 border border-[#D8D2C7] hover:border-[#1A1A1A] text-xs font-mono uppercase tracking-wider text-[#5C5855] hover:text-[#1A1A1A] transition-colors cursor-pointer"
              >
                Reset
              </button>
            )}
            ...
            <button
              ...
              className={`w-full sm:w-auto px-6 py-3.5 text-xs font-mono uppercase tracking-widest font-bold flex items-center justify-center gap-2 cursor-pointer transition-all ${
                !resumeText.trim() || isLoading || isUploading
                  ? "bg-[#D8D2C7] text-[#8C8477] cursor-not-allowed"
                  : "bg-[#1A1A1A] text-[#F7F5F0] hover:bg-[#B91C1C] active:translate-y-0.5"
              }`}
            >
              {isLoading ? (
                <>
